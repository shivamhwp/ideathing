...
export function KanbanBoard() {
	...
	const resolveCurrentPosition = () => {
		const firstColumnIdea =
			conceptColumn[0]?._id ?? toStreamColumn[0]?._id ?? null;
		const currentId = focusedIdeaId ?? firstColumnIdea;
		if (!currentId) return null;

		const conceptIndex = conceptColumn.findIndex(
			(idea) => idea._id === currentId,
		);
		if (conceptIndex >= 0)
			return { column: "concept" as const, index: conceptIndex };

		const toStreamIndex = toStreamColumn.findIndex(
			(idea) => idea._id === currentId,
		);
		if (toStreamIndex >= 0)
			return { column: "to-stream" as const, index: toStreamIndex };

		if (conceptColumn.length > 0)
			return { column: "concept" as const, index: 0 };
		if (toStreamColumn.length > 0)
			return { column: "to-stream" as const, index: 0 };
		return null;
	};
	...
	const moveVertical = (direction: 1 | -1) => {
		...
		const columnIdeas =
			position.column === "concept" ? conceptColumn : toStreamColumn;
		if (columnIdeas.length === 0) return;

		const targetIndex = Math.max(
			0,
			Math.min(columnIdeas.length - 1, position.index + direction),
		);
	...
	const moveHorizontal = (direction: 1 | -1) => {
		...
		const targetIdeas =
			targetColumn === "concept" ? conceptColumn : toStreamColumn;
	...
	return (
		<div className="flex h-full flex-col flex-1 min-h-0 gap-4 select-none">
			...
			<AddIdeaModal
				open={!isInteractionLocked && isAddModalOpen}
			...
			{!isInteractionLocked && selectionMode && selectedIds.length > 0 && (
				<div className="fixed inset-x-4 bottom-4 z-40 animate-in fade-in duration-150 sm:inset-x-auto sm:bottom-8 sm:right-8">
					<div className="w-full rounded-3xl border border-border/70 bg-popover/95 p-4 shadow-2xl backdrop-blur-xl sm:w-[280px]">
						<div className="flex items-center justify-between gap-2 text-xs text-muted-foreground pb-3 ">
							<div className="flex items-center justify-center gap-2">
								<span className="text-xs">Selected</span>
								<Badge
									variant="secondary"
									className="text-xs rounded-sm bg-secondary/40 text-secondary-foreground"
								>
									{selectedIds.length}
								</Badge>
							</div>
