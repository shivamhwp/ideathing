...
export function TheoIdeaQueue() {
	...
	const activeIdea =
		orderedIdeas.find((idea) => idea._id === editIdeaId) ?? null;
	...
	const moveFocus = (step: number) => {
		...
		const currentIndex = orderedIdeas.findIndex(
			(idea) => idea._id === focusedIdeaId,
		);
		const baseIndex = currentIndex >= 0 ? currentIndex : 0;
		const nextIndex = Math.max(
			0,
			Math.min(orderedIdeas.length - 1, baseIndex + step),
		);
	...
	useHotkey(
		...
		() => {
			...
			const focusedIdea = orderedIdeas.find(
				(idea) => idea._id === focusedIdeaId,
			);
