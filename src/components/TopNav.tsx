...
export function TopNav() {
	...
	const baseLink =
		"text-foreground/40 hover:text-primary text-sm font-semibold transition-colors";
	const boardLink = cn(
		baseLink,
		"flex items-center gap-2 font-semibold text-3xl",
	);
	const boardLinkActive = cn(boardLink, "text-primary font-semibold text-3xl");
	const recordedLink = cn(
		baseLink,
		"p-1 rounded-md font-semibold duration-200 text-foreground/40 transition",
	);
	const recordedLinkActive = cn(
		recordedLink,
		"font-semibold text-foreground text-primary",
	);
			...
			<div className="flex min-w-0 items-center gap-3 sm:gap-4">
				<Authenticated>
					<Link
						...
						className={cn(boardLink, "font-display")}
