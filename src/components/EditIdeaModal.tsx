...
function useScheduledUpdate(organizationId?: string) {
	...
	const { mutate: updateIdea, isPending, isSuccess } = useMutation({
				...
				{resourceList.map((resource, index) => (
					<div key={`resource-${index}`} className="flex items-center gap-2">
						...
						<Button
							...
							disabled={
								resourceList.length === 1 && resourceList[0]?.length === 0
							}
...
const RecordedUnsponsoredSection = memo(function RecordedUnsponsoredSection({ scheduleUpdate }: FieldProps) {
	const [recorded, setRecorded] = useAtom(editIdeaRecordedAtom);
		...
		<div className="flex items-center justify-between gap-4 pt-1">
			<div className="flex items-center gap-2">
				<Switch
					...
					checked={recorded}
					onChange={(e) => {
						setRecorded(e.target.checked);
						scheduleUpdate({ recorded: e.target.checked });
					}}
				/>
				<Label
...
const LabelStatusSection = memo(function LabelStatusSection({ scheduleUpdate }: FieldProps) {
	...
	const setRecorded = useAtom(editIdeaRecordedAtom)[1];
	...
	const handleStatusChange = (value: string) => {
		...
		if (value === "To Stream") {
			...
			setRecorded(true);
			scheduleUpdate({ status: value, recorded: true });
		} else {
			scheduleUpdate({ status: value || undefined, column: "ideas" });
		}
	};
			...
			<div className="space-y-1.5">
				...
				>
					...
					<SelectContent>
						...
						<SelectItem value="To Record(Off stream)">
...
const PotentialAdReadSection = memo(function PotentialAdReadSection({ scheduleUpdate }: FieldProps) {
...
export function EditIdeaModal({
	...
	const recorded = useAtomValue(editIdeaRecordedAtom);
					...
					<div className="px-6 py-5 space-y-5">
						...
						<div className="grid grid-cols-2 gap-4">
							...
							<div className="space-y-4">
								...
								<RecordedUnsponsoredSection scheduleUpdate={scheduleUpdate} />
							</div>
						</div>
