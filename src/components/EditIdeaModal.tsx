...
import {
	formatDateValue,
	parseDateValue,
} from "@/components/idea-form/date-utils";
import {
...
const ThumbnailSection = memo(function ThumbnailSection({
	...
	const [thumbnailReady, setThumbnailReady] = useAtom(
		editIdeaFields.thumbnailReady,
	);
...
const DatesSection = memo(function DatesSection({
	...
	const [vodRecordingDate, setVodRecordingDate] = useAtom(
		editIdeaFields.vodRecordingDate,
	);
...
const UnsponsoredSection = memo(function UnsponsoredSection({
	scheduleUpdate,
}: FieldProps) {
...
const LabelPotentialAdReadSection = memo(function LabelPotentialAdReadSection({
	...
	const [adReadTracker, setAdReadTracker] = useAtom(
		editIdeaFields.adReadTracker,
	);
...
export function EditIdeaModal({
	...
	const sendIdeaToNotionAction = useConvexAction(
		api.notion.actions.sendIdeaToNotion,
	) as (args: { ideaId: Idea["_id"] }) => Promise<{
		success: true;
		status: "queued" | "already_sent" | "already_sending";
	}>;
	const { mutate: sendIdeaToNotion, isPending: isSendingToNotion } =
		useMutation({
	...
	useHotkey(
		"N",
		(event) => {
			if (event.isComposing || isEditableTarget(event.target) || !isSHeld)
				return;
			...
			>
				...
				{isEditing ? (
					<div className="min-h-0 flex-1 space-y-2 overflow-hidden px-4 py-4 sm:px-6 sm:py-5">
						...
						{isTheoMode ? (
							<LabelPotentialAdReadSection scheduleUpdate={scheduleUpdate} />
						) : null}
