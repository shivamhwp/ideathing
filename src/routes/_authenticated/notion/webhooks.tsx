...
export const Route = createFileRoute("/_authenticated/notion/webhooks")({
	component: NotionWebhooksPage,
});

function NotionWebhooksPage() {
	const convexUrl = (import.meta as any).env.VITE_CONVEX_URL as
		| string
		| undefined;
			...
			<div className="max-w-3xl mx-auto space-y-6">
				...
				<div className="rounded-2xl border border-border/60 bg-card/40 p-5 space-y-4">
					...
					<div className="rounded-lg border border-border bg-muted/40 px-3 py-2 font-mono text-xs text-foreground break-all">
						{webhookUrl ??
							"Set VITE_CONVEX_URL to see the full webhook URL here."}
					</div>
					<p className="text-[11px] text-muted-foreground">
						If your Convex URL ends in{" "}
						<span className="font-mono">.convex.cloud</span>, replace it with{" "}
						<span className="font-mono">.convex.site</span> for webhooks.
					</p>
				</div>
				...
				<div className="rounded-2xl border border-border/60 bg-card/40 p-5 space-y-4 text-sm text-muted-foreground">
					...
					<ol className="list-decimal list-inside space-y-2">
						...
						<li>
							Subscribe to events:{" "}
							<span className="font-mono">page.properties_updated</span> (and
							optionally <span className="font-mono">page.created</span> /{" "}
							<span className="font-mono">page.content_updated</span>).
						</li>
						<li>
							Complete the Notion verification step and copy the verification
							token.
						</li>
						<li>
							Set{" "}
